...
"""%s
 ...
 - Warning    : If no result show use airplane mode
 - whatsap     : +923470000000
 - Note      : I love Pakistan
 - script name :  (Facebook Pakistan)
 - version     : 8.1
        ...
        if unikers in ('Y', 'y'):
            ...
            while True:
                ...
                if pwx == '':
                    ...
                    print ' [%s01%s] b-api (crack)'%(K,P)
                    print ' [%s02%s] mbasic (crack)'%(K,P)
                    print ' [%s03%s] mobile (crack sangat) '%(K,P)
                    zona(pwx.split(','))
                    break
        elif unikers in ('T', 't'):
            print '\n%s [ pilih methode crack - silahkan coba satu² ]\n'%(P)
            print ' [%s01%s] api (crack)'%(K,P)
            print ' [%s02%s] mbasic (crack)'%(K,P)
            print ' [%s03%s] mobile (crack) '%(K,P)
            self.langsung()
        else:
...
def menu():
    ...
    banner()
    print ('\n %s[ welcome %s%s%s ] \n'%(P,K,nama,P))
    print (' [%s01%s] Cloning from id public'%(K,P)) 
    print (' [%s02%s] Cloning from id followers'%(K,P)) 
    print (' [%s03%s] Cloning from id likes'%(K,P)) 
    print (' [%s04%s] %sStart crack %s'%(K,P,H,P)) 
    print (' [%s05%s] Ganti user agent'%(K,P)) 
    print (' [%s06%s] Cek hasil crack'%(K,P)) 
    #print (' [%s07%s] group'%(K,P))
